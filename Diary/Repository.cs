...
namespace Diary
{
    public class Repository
    {
        ...
        public static void UpdateRate(Student student, List<Rating> newRatings, ApplicationDbContext context, List<Rating> studentsRatings, Subject subject)
        {
            var subRaings = studentsRatings.Where(x => x.SubjectId == (int)subject).Select(x => x.Rate);

            var newSubRaings = newRatings.Where(x => x.SubjectId == (int)subject).Select(x => x.Rate);

            var subRatingToDelete = subRaings.Except(newSubRaings).ToList();
            var subRatingToAdd = newSubRaings.Except(subRaings).ToList();
